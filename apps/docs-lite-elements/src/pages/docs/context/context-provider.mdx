...
You can use the `stringifiedAttribute` function to do this.

The `stringifiedAttribute` function will convert the data to a string but being sure that the data will be parsed correctly when using JSX in a typescript environment.
...
<CodeBlock lang='html'>{`<my-context hydrate-context={stringifiedAttribute<MyContext>(data)}>`}</CodeBlock>
