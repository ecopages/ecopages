...
This is the html markup of the `LiteTodoApp` component.
...
<CodeBlock lang="html">{`<lite-todo-app class="todo" hydrate-context={stringifiedAttribute<Partial<TodoContext>>({ todos: data.todos })}>
...
export class LiteTodos extends WithKita(LiteElement) {
  ...
  @provideContext<typeof todoContext>({ context: todoContext, initialValue: { todos: [] }, hydrate: Object })
...
- `@reactiveAttribute` decorator to define the complete attribute
