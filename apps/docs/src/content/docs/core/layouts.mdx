...
export const Html = eco.html<JsxRenderable>({
...
**Why:** `eco.page({ layout })` merges layouts into `dependencies.components`. At render time, EcoPages validates that every dependency entry is a **declared** component with bound `config.identity` (`eco.component()`, `eco.layout()`, or `eco.html()`). Undeclared values throw `UndeclaredComponentDependencyError`.
...
export const BaseLayout = eco.layout<JsxRenderable>({
...
`eco.layout()` is typed as `LayoutProps` (children plus request context). Layouts that need extra typed props (`class`, `prose`) use `eco.component<YourProps>()`: that is still a valid `layout` value. Official templates do this for chrome props passed through `layout: { component, props }`.
...
On React sites with [`@ecopages/react-router`](/docs/ecosystem/react-router), `persistLayouts` is enabled by default. Each layout tier is cached by `config.identity.file` (or `id`). Two routes such as `[AppShell, Docs]` and `[AppShell, Settings]` share one mounted **AppShell** during client navigation; only the inner tier swaps. React state in the outer layout survives SPA transitions.

Provider layouts that mount shared client state (query clients, MobX stores, audio engines) should also set `runtimeProvider: true` on `eco.layout()` so Ecopages vendors one shared copy of those npm packages across page chunks. See [Shared runtime vendors](/docs/integrations/react#shared-runtime-vendors) in the React integration guide.
...
Import co-located CSS. List browser scripts explicitly. Direct local component imports are discovered automatically:
...
Use explicit `dependencies.stylesheets` only when you need attributes the import cannot express (for example `media="print"`). That declaration overrides the discovered file.

Every entry in `dependencies.components` must be a declared eco component, same as on pages. Use explicit `dependencies.components` only when discovery cannot follow the import (`export *` barrels, packages).
