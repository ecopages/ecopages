...
`loadEcoPagesConfig()` (via `ConfigBuilder.build()`) is the app-owned finalization boundary. Users author configuration using `defineConfig()`, which is finalized into the app-owned configuration at startup.
