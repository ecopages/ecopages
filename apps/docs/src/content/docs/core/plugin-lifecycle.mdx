...
`loadEcoPagesConfig()` (via `ConfigBuilder.build()`) is the app-owned finalization boundary. Users author configuration using `defineConfig()`, a typed identity helper that does not resolve paths or start plugins. `createApp()` loads that object and finalizes it at startup.
