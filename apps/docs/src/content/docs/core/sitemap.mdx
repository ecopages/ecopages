...
<RuiAlert variant="info" layout="banner" class="unstyled">
	<RuiAlertTitle>Production / static export only</RuiAlertTitle>
	<RuiAlertDescription>
		<p>
			Ecopages writes <code>sitemap.xml</code> during <code>ecopages build</code> (into <code>dist/</code>).{' '}
			<code>ecopages dev</code> does not generate or serve it. To inspect the file locally, build and use{' '}
			<code>ecopages preview</code>, or open <code>dist/sitemap.xml</code>.
		</p>
	</RuiAlertDescription>
</RuiAlert>
...
export default defineConfig({
	...
	baseUrl: process.env.ECOPAGES_BASE_URL ?? 'https://example.com',
	sitemap: {
		enabled: true,
		extraUrls: ['/rss.xml'],
		exclude: ['/admin/**'],
...
After `ecopages build`, inspect `dist/sitemap.xml` or run `ecopages preview`. `ecopages dev` does not generate or serve `/sitemap.xml` (the path 404s).

```bash
ECOPAGES_BASE_URL=https://example.com pnpm build
