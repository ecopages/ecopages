...
The Ecopages **dev toolbar** is an optional development-only browser inspector from `@ecopages/dev-toolbar`. It mounts during `ecopages dev` and does not ship to production builds.
...
| `integration` | Owning Integration plugin name for that page file (for example `ecopages-jsx`, `react`, `lit`) |
...
export default defineConfig({
	rootDir: import.meta.dirname,
	devToolbar: defineDevTool('@acme/my-dev-toolbar'),
	integrations: [ecopagesJsxPlugin()],
});
```
...
- [Packages](/docs/ecosystem/packages): where this package sits in the ecosystem
