...
2. Wrap the host in `eco.component()` and declare the script with `ssr: true` so Ecopages evaluates it on the server before SSR. Use `import type` from the script module when you only need props. You do not need a separate value import in the wrapper file.
