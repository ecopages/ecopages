...
<RuiAlert variant="info" layout="banner" class="unstyled">
	<RuiAlertTitle>Semantic filenames, no config setters</RuiAlertTitle>
	<RuiAlertDescription>
		The document shell and error pages resolve by basename and integration extension: `src/includes/html.*` and
		`src/pages/400.*`, `401.*`, `403.*`, `404.*`, `409.*`, and `500.*`. There is no filename setter in
		`eco.config.ts`. If neither a filesystem error page nor the matching `app.*()` loader is configured, the page
		pipeline serves built-in HTML for that status. See [Routing](/docs/core/routing).
	</RuiAlertDescription>
</RuiAlert>
...
<ApiField name="devToolbar" type="DevToolbarConfig" setter="setDevToolbar">
	Development-only in-browser inspector. Install `@ecopages/dev-toolbar` and call `devToolbar()`. See
	[Dev toolbar](/docs/ecosystem/dev-toolbar).
</ApiField>
