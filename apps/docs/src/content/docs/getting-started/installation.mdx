...
This example installs the core package along with the Ecopages JSX integration and its peer runtimes. That is the preferred stack. Add React, Lit, standalone MDX, or KitaJS only when a route needs that renderer.
