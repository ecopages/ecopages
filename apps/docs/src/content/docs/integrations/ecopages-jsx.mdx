...
Ecopages JSX is the preferred rendering integration. `@ecopages/ecopages-jsx` owns standard `.tsx` routes through the `@ecopages/jsx` runtime, with optional Radiant web components and MDX compiled against the same runtime. Start new apps here. Add React, Lit, or KitaJS only when a route needs that renderer.
