...
The `@ecopages/lit` package renders [Lit](https://lit.dev/) components on the server and registers their custom elements in the browser.
...
Lit preloads `dependencies.scripts` entries with `ssr: true` on the server before render, whether or not the entry is lazy in the browser. `lazy` controls browser timing only; `ssr: true` does not add an extra eager browser script for lazy entries. Omit `lazy` when the client script should load as soon as the page loads. The wrapper provides a typed interface for using the Lit element in pages.
...
Lit components with client scripts stamp [island host attributes](/docs/core/island-hosts) on their SSR root.
...
Use the Lit Integration when Pages or interactive islands should be standard Web Components.
