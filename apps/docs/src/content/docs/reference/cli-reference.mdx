...
| `-c, --config <file>`      | `ECOPAGES_CONFIG_FILE`  | Ecopages config file (default `eco.config.ts`)            |
...
2. **`baseUrl` in `eco.config.ts`**: Set directly via `defineConfig({ baseUrl: '...' })`.
...
When the preferred port is already in use, `ecopages dev` and `ecopages preview` can bind the next free port. TTY sessions ask for confirmation. Non-interactive sessions scan automatically only for the default port; a port pinned with `--port` or `ECOPAGES_PORT` fails instead.
...
<CodeTabs
	...
	tabs={[
		...
		{ id: 'pnpm', label: 'pnpm', code: 'pnpx ecopages dev' },
...
```bash
ECOPAGES_BASE_URL=https://my-site.com pnpx ecopages build
```

### Start the production server

```bash
pnpx ecopages start --runtime node
```
