...
When a Page combines its own relative assets with a content entry, use `mergePageDependencies()`. Do not object-spread the two bags: spread copies `ownerFile` onto the other side's relative paths. Page-owned CSS can still be a side-effect import; the merge bag is for the other file's contributions.
...
export default eco.page({
	dependencies: async ({ props }) =>
		mergePageDependencies({}, await getEntryDependencies(props.slug)),
	render: () => <article />,
});
```

Browser scripts remain explicit. Entries can be a path string or an object:

| Field | Role |
| --- | --- |
| `src` | Module path (relative to the declaring file, or aliased) |
| `content` | Inline script source (theme bootstrap, etc.) |
| `ssr` | Preload on the server before render (Lit / Ecopages JSX Radiant hosts) |
| `lazy` | Browser delivery trigger (`on:idle`, `on:visible`, `on:interaction`) |
| `attributes` | Extra `<script>` attributes (`defer`, `data-eco-rerun`, …) |
...
Use `eco.component()` when a component owns scripts, stylesheets, discovered child components, or lazy-loading behavior. Plain JSX functions remain useful for markup that does not own those dependencies.
...
- **Dependency Management:** Discovers local CSS and Eco Component imports; injects scripts you list in `dependencies.scripts`.
- **Lazy Loading:** Supports interaction, visibility, and idle triggers on those scripts.
- **Type Safety:** Infers props for both internal usage and custom elements.


For components that need scripts or lazy loading:
...
> **Note:** If your component requires a dedicated CSS file, import it next to the component (`import './card.css'`) or use `eco.component()` so discovery can attach it. Do not rely on a plain function to own a stylesheet.
...
Use `eco.component()` with React to manage dependencies and hydration while retaining type safety.
...
Creates the document shell component: the outermost HTML wrapper rendered once per page. Semantically equivalent to `eco.component()` but signals that this component owns the document (`<html>`, `<head>`, `<body>`).

`eco.html<E>()` takes **one** generic: the renderable. Props are `HtmlTemplateProps` (`children`, `metadata`, `headContent`, `language`, `pageProps`, `pageModuleUrl`).
...
export const Document = eco.html<JsxRenderable>({
...
`eco.layout<E>()` is typed as `LayoutProps<E>` (children plus request context). Extra chrome props (`class`, `prose`) are typed with `eco.component<YourProps, E>()`; both are valid `layout` values.
...
On React sites with [`@ecopages/react-router`](/docs/ecosystem/react-router), each tier is cached independently during SPA navigation. Routes that share an outer layout: for example `[AppShell, Docs]` and `[AppShell, Settings]`: reuse the same mounted outer instance while inner tiers swap.
...
Use `EcoEmbed` (or pre-serialized HTML) for cross-integration shell stacks. See [Ecopages JSX: Mixed Rendering](/docs/integrations/ecopages-jsx#mixed-rendering) for the full boundary contract.
...
// html() takes one generic `E` (the renderable). Props are HtmlTemplateProps.
// layout() takes `E` as well; extra typed chrome props use eco.component() instead.
