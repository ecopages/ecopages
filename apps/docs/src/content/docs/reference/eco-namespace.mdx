...
>
> **Important:** Plain React functions are **not** valid as `eco.page({ layout })` values or as `dependencies.components` entries. The renderer validates that dependency graph nodes are **declared** components (`eco.component()`, `eco.layout()`, or `eco.html()`) with injected `config.__eco` metadata. Use `eco.layout()` for route wrappers instead of ad-hoc functions.
