...
Ecopages provides a streamlined way to define API endpoints within your application. This allows you to build server-side logic, fetch data, or perform actions directly from your Ecopages project.
...
app.get('/api/example/:id', async (context) => {
  ...
  const { request, response, appConfig } = context;
...
-   `appConfig`: The fully resolved Ecopages application configuration object.
