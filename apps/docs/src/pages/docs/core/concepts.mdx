...
When fetching data in `getStaticPaths` or `getStaticProps`, Ecopages provides two approaches:

### Using Runtime Origin

The `runtimeOrigin` parameter is particularly powerful for complex scenarios where you need the full server URL:

<CodeBlock>
```typescript
export const getStaticPaths: GetStaticPaths = async ({ runtimeOrigin }) => {
  // Full URL for complex scenarios
...
Using `runtimeOrigin` is beneficial when:
- Your API routes need the complete server URL
- You're dealing with preview environments
- You have different API endpoints for different environments
- You need to handle cross-origin requests during development

### Using Relative Paths

For simpler scenarios, you can use relative paths:

<CodeBlock>
```typescript
export const getStaticPaths: GetStaticPaths = async () => {
  // Simple relative path works fine for basic cases
...
Relative paths are suitable when:
- Your API is always on the same origin
- You don't need environment-specific URLs
- You want simpler, more portable code
