...
- `config.layout` — innermost tier (legacy alias)
...
## File-system segment layouts

v0.2 does **not** infer layouts from directory structure (for example `layouts/docs.tsx` wrapping `pages/docs/*`). Layout assignment is explicit on each `eco.page()` call. See the [segment layout inheritance ADR](https://github.com/ecopages/ecopages/blob/main/docs/adr/segment-layout-inheritance.md) for the proposed v2 model.
