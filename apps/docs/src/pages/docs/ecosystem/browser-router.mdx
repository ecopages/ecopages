...
<CodeTabs
	tabs={[
		{ id: 'pnpm', label: 'pnpm', code: 'pnpm dlx jsr add @ecopages/browser-router' },
		{ id: 'npm', label: 'npm', code: 'npx jsr add @ecopages/browser-router' },
		{ id: 'bun', label: 'bun', code: 'bunx jsr add @ecopages/browser-router' },
	]}
...
| `viewTransitions` | `boolean` | `true` | Enables [View Transitions API](https://developer.mozilla.org/en-US/docs/Web/API/View_Transitions_API) support. |
...
<Banner type="info">
  <Banner.Title>CSS exported via ecopages npm package</Banner.Title>
  <p>Due to a <a href="https://github.com/jsr-io/jsr/issues/293">JSR limitation</a>, CSS files cannot be exported from JSR packages. View transition styles are shipped via the <code>ecopages</code> npm package instead.</p>
</Banner>
