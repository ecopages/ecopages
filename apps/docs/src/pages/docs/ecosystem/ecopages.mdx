...
For Ecopages apps, prefer these commands over calling `bun run app.ts` flags directly.

| Command | Purpose |
| :--- | :--- |
| `ecopages init <dir>` | Scaffold a new Ecopages app |
| `ecopages dev [entry]` | Start dev server |
| `ecopages dev:watch [entry]` | Start dev server with restart-on-change |
| `ecopages dev:hot [entry]` | Start dev server with hot reload |
| `ecopages build [entry]` | Build production output |
| `ecopages preview [entry]` | Preview production build |
| `ecopages start [entry]` | Start production server |

> **Note:** `[entry]` defaults to `app.ts`.

## Environment Overrides

Server commands support flags that map to environment variables:
