...
- **Bun Optimized**: Uses `Bun.Glob`, `Bun.hash`, `Bun.file`, and `Bun.write` for maximum performance
- **Node.js Fallback**: Uses native `node:fs/promises.glob()` and `crypto` for full compatibility
- **Type Safe**: Consistent TypeScript interface across runtimes

## Installation

<CodeTabs
	name="file-system-install"
	tabs={[
		{ id: 'pnpm', label: 'pnpm', code: 'pnpm add @ecopages/file-system' },
		{ id: 'npm', label: 'npm', code: 'npm install @ecopages/file-system' },
		{ id: 'bun', label: 'bun', code: 'bun add @ecopages/file-system' },
	]}
