...
<CodeSnippetTabs>
  <CodeTab name="pnpm">
```bash
pnpm dlx jsr add @ecopages/postcss-processor
```
  </CodeTab>
  <CodeTab name="npm">
```bash
npx jsr add @ecopages/postcss-processor
```
  </CodeTab>
  <CodeTab name="bun">
```bash
...
This plugin automatically registers a Bun loader that processes your CSS files on import.
