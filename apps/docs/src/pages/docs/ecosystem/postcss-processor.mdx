...
This preset supports the newer Tailwind CSS v4, utilizing `@tailwindcss/postcss`. It also automatically handles `@reference` injection for `@apply` directives, making usage simpler in isolated component styles.
...
postcssProcessorPlugin({
  ...
  transformInput: async (css, filePath) => {
    // Modify CSS before processing
    return css;
  },
  transformOutput: async (css) => {
    // Modify CSS after processing
    return css;
  }
})
```
