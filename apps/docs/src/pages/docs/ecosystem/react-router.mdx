...
- **Standard links** - Works with regular `<a>` tags, no special components needed
...
3. **Navigation**: When a link is clicked:
   ...
   - Extracts the component module URL and props
   - Dynamically imports the new page component
   - Updates the head (title, meta, styles)
   - Renders the new page within the existing layout
