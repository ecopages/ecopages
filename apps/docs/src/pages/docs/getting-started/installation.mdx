...
<CodeSnippetTabs>
  <CodeTab name="pnpm">
```bash
pnpm dlx jsr add @ecopages/core @ecopages/kitajs @ecopages/mdx
```
  </CodeTab>
  <CodeTab name="npm">
```bash
npx jsr add @ecopages/core @ecopages/kitajs @ecopages/mdx
```
  </CodeTab>
  <CodeTab name="bun">
```bash
...
<Banner type="info">
  <Banner.Title>JSR Configuration</Banner.Title>
  <p>
    To install packages from the JSR registry, you'll need to configure your npmrc. Create an `.npmrc` file in your project root with the following content:
  </p>
  ```properties
...
- `pnpm run dev` (or `bun run dev`): Starts the development server (with HMR). You can override the host/port with environment variables.
- `pnpm run build` (or `bun run build`): Builds your project for production into the `.eco` directory. Uses the configured `baseUrl`.
- `pnpm run start` (or `bun run start`): Serves the production build from the `.eco` directory. Uses the configured `baseUrl`.
- `pnpm run preview` (or `bun run preview`): Builds and serves the production build locally for previewing.
