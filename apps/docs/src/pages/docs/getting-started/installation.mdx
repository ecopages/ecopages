...
- `pnpm run build` (or `bun run build`): Builds your project for production into the `.eco` directory.
- `pnpm run start` (or `bun run start`): Starts the production server from the existing build output.
- `pnpm run preview` (or `bun run preview`): Builds and then serves the production build locally for verification.
