...
export const getMetadata = () => ({
  ...
  description: 'Learn how to use the Lit integration with Eco Pages',
})

# Lit Integration

The `@ecopages/lit` package provides seamless integration with [Lit](https://lit.dev/), enabling the use of Lit components and reactive properties within the Eco Pages framework.
...
To incorporate the Lit integration into your Eco Pages project, follow these steps:

1. Import the `litPlugin` in your Eco Pages configuration file:
...
1. **EcoWebComponent**: Extend from `EcoWebComponent` to create Lit components that integrate seamlessly with Eco Pages.
2. **Server-Side Rendering (SSR)**: Lit components are server-side rendered by default in Eco Pages, except when used within MDX files (currently).
...
The Lit integration works well with other Eco Pages plugins. For example, you can use it alongside the kitajs plugin to create reusable components in a jsx style.
...
By leveraging the Lit integration and `EcoWebComponent`, you can create powerful, reactive components within your Eco Pages project, combining the efficiency of static site generation with the interactivity of modern web components.