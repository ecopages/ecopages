...
4. **TypeScript Support**: Full TypeScript support with `EcoReactComponent` type.

## Limitations

- Currently, MDX is not supported when using only the React integration.
- Server-side rendering capabilities may be limited compared to other integrations.
...
3. **TypeScript**: Use the `EcoReactComponent` type for better type checking.