...
Use `createForeignChildRuntime()` for custom interception logic, or delegate to `createQueuedForeignSubtreeResolutionRuntime()` when the renderer should queue foreign subtrees and resolve them after the local shell render completes.

Integrations such as Lit use this pattern because the owning renderer needs to re-enter during foreign-subtree resolution to produce the correct SSR output.
...
class CustomIntegration extends IntegrationPlugin {
	...
	override get browserBuildPlugins() {
		const runtimeAliasPlugin = createRuntimeSpecifierAliasPlugin(
			{
		...
		return runtimeAliasPlugin ? [runtimeAliasPlugin] : [];
	}
...
Use `browserBuildPlugins` when the integration needs browser-only transforms or stable bare-specifier aliases in browser bundles. Override `setHmrManager()` only when behavior beyond the shared strategy registration path is required.
...
4. Use `browserBuildPlugins` for browser-only transforms and runtime specifier aliasing in browser bundles.
5. Fall back to full reload when a hot update is not safe. Correctness matters more than partial HMR.
6. Document any required foreign-child ownership rules. Cross-integration rendering is the first place future readers will get lost.
