...
If your integration depends on a specific runtime, declare that in `runtimeCapability` so Ecopages can fail during config finalization instead of during startup.
...
```
	...
	If you override `getRuntimeSpecifierMap()`, Ecopages registers those mappings automatically when the integration is attached to the active HMR manager. Today they are used by the development runtime and browser bundle aliasing path, but the registry is intentionally generic enough to support broader import-map-style behavior later. Avoid overriding `setHmrManager()` unless you need behavior beyond the shared registration and strategy setup path.
