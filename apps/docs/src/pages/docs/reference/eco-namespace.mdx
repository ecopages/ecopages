...
Pass a single layout or an **array** of layouts. Array order is **outermost first**, innermost last — the same mental model as segment nesting in App Router-style frameworks.
...
At factory time, EcoPages normalizes this stack onto `config.layouts` (components) and `config.layoutEntries` (components plus optional per-tier `props` factories). The innermost tier is also exposed as `config.layout` for legacy call sites.
