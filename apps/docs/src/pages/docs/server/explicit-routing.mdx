...
This example shows how Standard Schema allows using different libraries together. In practice, you would pick one library for your project:
...
app.post('/api/search', async (ctx) => {
	const { q } = ctx.validated.query;
	const filters = ctx.validated.body;
	return ctx.json({ query: q, filters });
}, {
	schema: {
		query: z.object({ q: z.string() }),
		body: v.object({ category: v.optional(v.string()) })
	}
});
```
