...
<Banner type="info">
  <Banner.Title>Preload Configuration Required</Banner.Title>
  <p>When using explicit routes, integrations are loaded after the app entry point. To ensure they initialize correctly, you <strong>must</strong> preload your configuration file.</p>
  <p>Add your config file to the <code>preload</code> array in your <code>bunfig.toml</code>:</p>
   ```toml
  preload = ["./eco.config.ts"]
  ```
</Banner>
