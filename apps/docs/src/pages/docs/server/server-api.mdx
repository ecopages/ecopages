...
`app.start()` boots the runtime for the current CLI mode (`dev`, `preview`, `build`, or production `start`). Pass an optional callback to run once the server can accept traffic:

```typescript
import { createApp, type OnAppStartCallback } from '@ecopages/core/create-app';

const onAppStart: OnAppStartCallback = ({ origin }) => {
	console.log(`Ecopages is ready at ${origin}`);
};

await app.start(onAppStart);
```

The callback receives `{ origin }` — the resolved base URL with no trailing slash (for example `http://localhost:3000`). It runs after the runtime bind completes, not when the process starts.

### Embedded hosts (Vite)

When Ecopages runs inside a host such as the [Vite plugin](/docs/ecosystem/vite-plugin), pass `runtime: { embedded: true }` to `createApp()`. In that mode `app.start()` only registers the callback; the host opens the port and calls `app.handleListening(origin)` when the app is ready to serve requests.
...
await app.start(({ origin }) => {
	console.log(`Embedded app ready at ${origin}`);
});
```

See [Build and Host](/docs/core/build-and-host) for how embedded runtime mode fits the host model.

### Readiness signals for tooling

For test runners or process supervisors that match stdout (for example Playwright `webServer`), log a machine-readable ready line from the callback:
...
This prints `[@ecopages/ready] http://localhost:3000`. Prefer waiting on that marker over probing `GET /` during boot — the port can open before SSR is warm.
