...
When using the Bun adapter, you can use the `defineApiHandler` helper function. This function automatically infers the parameter types from the `path` string literal, ensuring your `request.params` object is correctly typed without explicit annotations.

```typescript
// Import the adapter-specific helper
import { defineApiHandler } from '@ecopages/core/adapters/bun/define-handler'; // Adjust path if needed
