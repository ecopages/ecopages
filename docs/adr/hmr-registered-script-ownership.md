...
1. **`HmrEntrypointRegistrar` owns entrypoint state.** `registered` maps source paths to dev-transform URLs (`outputPath` equals `sourcePath` for on-demand modules). Legacy disk emit remains available for registrar unit tests only.
...
4. **`BrowserBundleService` is the sole browser-plugin resolver.** HMR managers do not store plugin copies. `hmr-runtime` and on-demand dev transform builds route through the `browser-hmr` executor profile.

5. **Registered-script invalidation is centralized in `SharedHmrManager.handleFileChange()`.** Server invalidation, integration hooks, bypass-cache import, cache invalidation, and broadcast run in one sequence. Embedded watchers and the Vite hot-update path delegate to the manager instead of duplicating Radiant SSR invalidation.

6. **Dev-transform plugin ownership uses `HmrStrategy.ownsDevTransformEntrypoint()`.** `DevTransformBundler` selects integration contributors from that hook. File-change dispatch continues to use `matches()` / `process()`.
...
- Cold-start E2E must verify layout scripts are served from `/assets/__eco_dev__/` before other HMR specs warm entrypoints.
