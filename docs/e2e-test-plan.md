...
Groups run in order: **preview → dev → hmr**. Tune isolated-server concurrency with `ECOPAGES_E2E_SERVER_CONCURRENCY` (preview cap; default `min(N, floor(N/2))`). Dev defaults to **1 parallel process** because read-only kitchen-sink dev/preview rows share `.e2e-tmp/kitchen-sink-shared`; raise with `ECOPAGES_E2E_DEV_CONCURRENCY` when you have headroom. Hmr stays at `1`.
