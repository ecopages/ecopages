...
test('NodeHmrManager registers script entrypoints with dev transform URLs without blocking builds', async () => {
...
test('NodeHmrManager broadcasts dev transform update when a watched page source changes', async () => {
	...
	assert.equal(broadcast.mock.calls[0]?.[0]?.type, 'update');
	assert.equal(broadcast.mock.calls[0]?.[0]?.path, `${DEV_TRANSFORM_URL_PREFIX}/pages/deleted-page.js`);
});
