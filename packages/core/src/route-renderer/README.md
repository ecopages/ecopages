...
Dependency collection reads the merged `config.dependencies` graph, including deferred direct-import Component references. Traversal visits each config once; multiple Components in one file remain distinct. It collects explicit styles before inferred styles across the graph, suppressing inferred duplicates by resolved path. Source watch paths include discovered Components and their stylesheet files.
