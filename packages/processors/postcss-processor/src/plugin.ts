    ...
    return [
      bunInlineCssPlugin({
        ...
        transform: async (contents: string | Buffer) => {
          const transformedContents = options?.transformInput ? await options.transformInput(contents) : contents;
    ...
    if (foundConfigPath) {
      try {
        ...
        if (postcssConfig && typeof postcssConfig.plugins === 'object' && postcssConfig.plugins !== null) {
          ...
          logger.warn(`PostCSS config file found (${foundConfigPath}), but no valid 'plugins' export detected.`);
        }
      } catch (error: any) {
