...
const commands = [
	...
	},
	{
		...
		description: 'Fetches the same semantic-shell catalog record that powers the static route under /catalog/[slug].',
		headers: {},
			...
			<div class="api-lab">
				<section class="api-lab__intro">
					...
					<p class="api-lab__summary">
						The kitchen sink keeps JSON handlers next to pages so route registration, middleware, locals, and error handling all live in one place.
					</p>
					<ul class="api-lab__route-list">
						<li><span class="api-lab__route-path">/api/v1/ping</span> returns request locals injected by middleware.</li>
						<li><span class="api-lab__route-path">/api/v1/echo</span> reflects JSON bodies and confirms POST handling.</li>
						<li><span class="api-lab__route-path">/api/v1/catalog/:slug</span> exposes the same data used by the static catalog page.</li>
						<li><span class="api-lab__route-path">/api/v1/admin/*</span> is protected by grouped middleware and shared error handling.</li>
					</ul>
				</section>
								...
								{commands.map((command, index) => (
									...
									>
										<div class="api-lab__command-header">
											...
											<span class="api-lab__command-cta">
												Run
											</span>
										</div>
										<p class="api-lab__command-description">{command.description}</p>
										<pre class="api-lab__command-code"><code>{command.curl}</code></pre>
									</button>
								))}
							</div>
						</div>
						<div class="api-lab__viewer" data-api-response-viewer>
							<div class="api-lab__viewer-header">
								<div>
									...
									<h2 class="api-lab__viewer-title" data-response-label>Select a command</h2>
								</div>
								<div class="api-lab__viewer-badges">
									<span class="api-lab__viewer-badge" data-response-status>Idle</span>
									<span class="api-lab__viewer-badge" data-response-time>0 ms</span>
								</div>
							</div>
							<div class="api-lab__viewer-sections">
								<div class="api-lab__viewer-section">
									<p class="api-lab__panel-label">Request</p>
									<pre class="api-lab__viewer-code"><code data-response-request>Click a command to send a request from the browser.</code></pre>
								</div>
								<div class="api-lab__viewer-section">
									<p class="api-lab__panel-label">Response body</p>
									<pre class="api-lab__viewer-code api-lab__viewer-code--body"><code data-response-body>Waiting for input.</code></pre>
								</div>
								<div class="api-lab__viewer-section">
									<p class="api-lab__panel-label">Headers</p>
									<pre class="api-lab__viewer-code"><code data-response-headers>No response headers yet.</code></pre>
								</div>
							</div>
						</div>
					</div>
