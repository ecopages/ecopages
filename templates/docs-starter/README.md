...
| `/docs-llm/<section>/<slug>.md` | Generated Markdown for one exported page           |
| HTML `rel="alternate"`          | Advertises the markdown URL from the docs pathname |

`pnpm run generate:llms` runs before `dev` and `build`. `ContentScanner` reads `src/content/docs`; the script converts UI-only MDX components to Markdown, writes `src/public/llms.txt`, and **replaces** `src/public/docs-llm/`. Deleted pages and `llms: false` entries are omitted from the index and removed from that tree.
