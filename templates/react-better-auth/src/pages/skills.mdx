...
export const getMetadata = () => ({
  title: "Ecopages Skills Guide",
  description: "A comprehensive guide for LLMs to create applications using Ecopages",
});
...
Ecopages is a TypeScript-first static site generator that runs on Node.js, with optional Bun. It is designed to be **simple, fast, and explicit**.

**Key Features:**
- **Node and Bun**: The CLI prefers Bun when you launch from Bun, otherwise it uses Node.
...
- **`eco.component()`**: Factory for reusable components. Use it when a component needs browser scripts, lazy loading, or mixed-integration ownership.
- **`eco.page()`**: Factory for page routes. Handles metadata, layouts, and static props.

In modules that declare `eco.component()`, `eco.layout()`, `eco.html()`, or `eco.page()`, Ecopages discovers:

- **Local components** from direct static imports
- **Stylesheets** from side-effect CSS imports such as `import './page.css'`

**What stays explicit:**

- **Browser scripts** in `dependencies.scripts`
- **`export *` barrels, package components, and other imports discovery cannot follow** in `dependencies.components`

> **Note:** When using **KitaJS**, components are often plain functions returning JSX strings. Reach for `eco.component()` when the component brings browser scripts or must participate in the dependency graph.
...
```bash
pnpm add @ecopages/core @ecopages/kitajs
```

Add PostCSS processor for Tailwind:

```bash
pnpm add @ecopages/postcss-processor
...
- Export `config` with `layout` when the page needs a wrapper
- Export `getMetadata` for page metadata
- Write markdown content below the exports
- Use JSX components inline with markdown syntax
- Top-level component imports are discovered automatically
...
Import co-located CSS directly. Ecopages discovers the stylesheet and strips the import during build:
...
When the component also needs browser behavior, keep scripts explicit in `dependencies.scripts`:
...
:root {
  ...
  --color-on-background: oklch(0.18 0.02 265);
}
```
...
export function Seo({
  ...
  return (
    <>
      <title>{title}</title>
      <link rel="icon" type="image/svg+xml" href="/favicon/favicon-dark.svg" media="(prefers-color-scheme: light)" />
      <link rel="icon" type="image/svg+xml" href="/favicon/favicon-light.svg" media="(prefers-color-scheme: dark)" />
...
2. **Import local CSS directly** - Side-effect CSS imports are discovered automatically.
3. **Keep browser scripts explicit** - Declare them in `dependencies.scripts`.
4. **Use `eco.component()` when scripts matter** - Not for every presentational wrapper.
5. **Prioritize native web standards** - Use standard anchor tags (`<a>`), forms, and semantic HTML.
6. **Leverage TypeScript** - Full type inference throughout.
7. **Organize by feature** - Group related components, handlers, and pages together.
8. **Use CSS variables for theming** - Works seamlessly with Tailwind v4.
9. **Avoid SSR-only hooks** - In React, use client-side checks for browser APIs.
...
- Import CSS with `import './file.css'` in the owning page, layout, or component
...
> **Important**: Files containing server secrets or sensitive logic MUST use the `.server.ts` extension. Ecopages relies on `.server.ts` to prevent server imports from leaking into the client bundle.
...
Create `src/handlers/auth.server.ts`:
...
export const protectedGroup = defineGroupHandler({
  ...
  routes: (define) => [
    define({
      ...
      handler: async (ctx) => {
        const { default: DashboardView } = await import("@/views/dashboard");
        return ctx.render(DashboardView, { user: ctx.locals.session?.user });
      },
    }),
  ],
});
...
- Pass data from the handler context (e.g., `ctx.locals.session?.user`) as props to the view
